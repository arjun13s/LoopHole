...
INSTRUCTIONS = (
    ...
    "  resource_misuse  wasted context/resources (e.g. after a concrete failure, read irrelevant docs/README/large context instead of targeted evidence)\n"
    "  tool_misuse      misused a tool or acted on its bad result (e.g. submitted after failed tests)\n"
    "  routing          skipped required verification before submit/end (not merely editing again after a failed test)\n"
    ...
    "(after a concrete failure reading irrelevant docs/README/oversized context instead of targeted evidence, skipping verification, submitting after "
    ...
    "Calibration: reading a README/doc before focused work can be normal context gathering. "
    "When localizing resource_misuse after an error, choose the wasteful read/context step itself, "
    "not the earlier failed test/error step. "
    "When tests reveal the intended implementation file was unchanged after an edit, "
    "call wrong_file_edit at the earlier write_file step that targeted the wrong path, "
    "not routing at the later failed test step. "
    "Do not call routing just because tests failed and the agent edited again; routing "
    "requires missing verification before submit/end or skipping a required check. "
    "Do not call tool_misuse for healthy test-fail -> diagnose -> fix -> rerun -> pass loops.\n"
